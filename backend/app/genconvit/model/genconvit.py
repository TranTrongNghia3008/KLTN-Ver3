    ...
    def __init__(self, config, ed, vae, net, fp16):
        ...
        if self.net=='ed':
            try:
                ...
                self.checkpoint_ed = torch.load(f'app/genconvit/weight/{ed}.pth', map_location=torch.device('cpu'))
                ...
                raise Exception(f"Error: app/genconvit/weight/{ed}.pth file not found.")
        elif self.net=='vae':
            try:
                self.model_vae = GenConViTVAE(config)
                self.checkpoint_vae = torch.load(f'app/genconvit/weight/{vae}.pth', map_location=torch.device('cpu'))
